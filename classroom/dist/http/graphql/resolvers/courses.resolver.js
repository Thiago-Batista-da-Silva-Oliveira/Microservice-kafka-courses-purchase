...
let CourseResolver = class CourseResolver {
    constructor(coursesService, studentsService, enrollmentsService) {
    ...
    async course(id, user) {
        const student = this.studentsService.getStudentByAuthUserId(user.sub);
        if (!student) {
...
CourseResolver = __decorate([
    (0, graphql_1.Resolver)(() => course_1.Course),
